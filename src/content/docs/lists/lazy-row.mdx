...
El `LazyRow` es un componente que fue agregado oficalmente en la versión 1.2 de Jetpack Compose, antes de este existió el `ScrollableRow` pero este ha sido removido de toda documentación oficial
por el mismo motivo de incentivar/obligar el uso `LazyRow`, previo a 1.2 existen referencias de este como un componente en desarrollo, cabe mencionar que `LazyRow` no pertenece a **material.io** por lo que sus variantes han sido las funciones de extensión que este permite
...
Continuando la explicación del componente, este crea y dispone los elementos que están visibles en el viewport del mismo, produciendo un desplazamiento horizontal, este sigue los principios de un `RecyclerView`
...
<TabItem label="LazyRow - itemsIndexed">
